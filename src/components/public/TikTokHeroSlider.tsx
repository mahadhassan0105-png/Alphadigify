...
import TikTokMockupShowcase from "./TikTokMockupShowcase";

const bgImages = ["/tiktok_bg_1.jpg", "/tiktok_bg_2.jpg"];

export default function TikTokHeroSlider() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [direction, setDirection] = useState(1);

  const nextSlide = useCallback(() => {
    setDirection(1);
    setCurrentIndex((prev) => (prev === tiktokSlides.length - 1 ? 0 : prev + 1));
  }, []);
    ...
    <section className="relative w-full h-auto min-h-0 sm:min-h-[700px] lg:h-[100vh] lg:min-h-[950px] [overflow-x:clip]">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden" style={{ clipPath: 'ellipse(120% 95% at 50% 0%)', backgroundColor: '#000' }}>
        <AnimatePresence initial={false}>
          <motion.div key={currentBg} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.8, ease: "easeInOut" }} className="absolute inset-0 w-full h-full" style={{ willChange: "transform, opacity" }}>
            <Image src={currentBg} alt="TikTok Hero Background" fill priority={currentIndex === 0} className="object-cover object-center" quality={90} />
          </motion.div>
        </AnimatePresence>
      </div>

      {/* Overlay */}
      <div className="absolute inset-0 z-[1] bg-gradient-to-b from-[#1a1a2e]/80 via-[#10101c]/60 to-[#0c0c16]/90" style={{ clipPath: 'ellipse(120% 95% at 50% 0%)' }} />
      ...
      <div className="relative z-10 flex flex-col items-center pt-32 sm:pt-24 lg:pt-36">
        ...
        <div className="w-full flex justify-center px-4 sm:px-6 lg:px-12 mt-0 sm:mt-0">
