...
function ShoppingBagIcon({ color }: { color: string }) {
  return <svg viewBox="0 0 24 24" fill="none" strokeWidth={1.8} stroke={color} className="w-7 h-7"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>;
}
function TikTokIcon() {
  return (
...
function GoogleDotsIcon() {
  return (
    <span className="inline-flex gap-[3px] items-center">
      ...
      <span style={{ width: 10, height: 10, borderRadius: "50%", background: "#FBBC05", display: "inline-block" }} />
    ...
    >
      ...
      <section className="relative pt-32 pb-20 lg:pt-44 lg:pb-32 overflow-hidden min-h-[90vh] flex items-center">
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-br from-black/85 via-[#0B0C10]/80 to-black/90" />
        ...
        {/* Glows */}
        <div className="absolute top-[10%] left-[5%] w-80 h-80 rounded-full blur-[120px] pointer-events-none" style={{ background: "rgba(250,204,21,0.07)" }} />
        <div className="absolute bottom-[5%] right-[10%] w-64 h-64 rounded-full blur-[100px] pointer-events-none" style={{ background: "rgba(250,204,21,0.05)" }} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center w-full">
          ...
          >
            {/* Badge */}
            <div className="inline-flex items-center gap-2 bg-yellow-400/10 border border-yellow-400/20 backdrop-blur-md px-4 py-2 rounded-full mb-8">
              <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" />
              <span className="text-sm font-semibold tracking-wide uppercase text-yellow-300">
                Reviews Management
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="/contact">
                <Button size="lg" className="bg-yellow-500 hover:bg-yellow-400 text-slate-900 h-14 px-8 text-base font-bold rounded-full transition-all hover:scale-105 shadow-lg shadow-yellow-500/25">
                  Build My Review Profile <ArrowRight className="ml-2 w-5 h-5" />
                </Button>
              </Link>
              <Link href="#platforms">
                <Button size="lg" variant="outline" className="h-14 px-8 text-base font-semibold rounded-full border-white/20 text-white hover:bg-white/10 transition-all">
                  See Platforms
                </Button>
              </Link>
            </div>
          ...
          {/* Right: Animated Review Dashboard Mockup */}
          <motion.div
          ...
          >
            <div className="bg-white dark:bg-[#1a1b26] rounded-3xl border border-slate-200 dark:border-slate-700/60 shadow-2xl overflow-hidden">
              {/* Dashboard header */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-[#1e2030]">
                <div className="flex items-center gap-3">
                  <Star className="w-5 h-5 text-yellow-400 fill-yellow-400" />
                  <span className="text-sm font-bold text-slate-700 dark:text-slate-300">AlphaDigify — Reviews Dashboard</span>
                </div>
              ...
              <div className="p-6">
                ...
                <div className="text-center mb-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                  <motion.p
                    ...
                    className="text-5xl font-black text-yellow-500 mb-2"
                  >
                    4.9
                  </motion.p>
                  <AnimatedStars rating={5} size={24} />
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 font-medium">Average across all platforms</p>
                </div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    >
                      <span className="text-xs font-bold text-slate-600 dark:text-slate-400 w-20 shrink-0">{item.platform}</span>
                      ...
                      <span className="text-xs font-black shrink-0" style={{ color: item.color }}>{item.rating}★</span>
                      <span className="text-[10px] text-slate-400 shrink-0">{item.reviews.toLocaleString()}</span>
                    </motion.div>
                  ))}
                </div>
                ...
                <motion.div
                  ...
                  className="mt-5 pt-5 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center"
                >
                  <span className="text-xs text-slate-500">Reviews this month</span>
                  <span className="text-sm font-black text-green-500">+312 ↑ 48%</span>
                </motion.div>
              </div>
            </div>
          </motion.div>
      ...
      ═══════════════════════════════════════════════ */}
      <section className="py-20 bg-white dark:bg-[#0B0C10]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 px-3 py-1 rounded-full text-xs font-bold mb-5">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { stat: "93%", label: "of buyers read reviews before purchasing", icon: <Eye className="w-7 h-7" />, color: "text-yellow-500", bg: "bg-yellow-50 dark:bg-yellow-500/10" },
              { stat: "4.0★+", label: "minimum rating required to convert modern shoppers", icon: <Star className="w-7 h-7" />, color: "text-yellow-500", bg: "bg-yellow-50 dark:bg-yellow-500/10" },
              { stat: "+9%", label: "revenue increase from every 1-star rating improvement", icon: <TrendingUp className="w-7 h-7" />, color: "text-yellow-500", bg: "bg-yellow-50 dark:bg-yellow-500/10" },
            ].map((item, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 30 }}
                ...
                className="text-center p-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm"
              >
                <div className={`w-14 h-14 ${item.bg} ${item.color} rounded-2xl flex items-center justify-center mx-auto mb-5`}>
                  {item.icon}
                </div>
                <p className="text-4xl font-black text-yellow-500 mb-3">{item.stat}</p>
                <p className="text-slate-600 dark:text-slate-400 font-medium text-sm leading-relaxed">{item.label}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {PLATFORMS.map((platform, i) => (
              <motion.div
                ...
                className="group relative bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-8 overflow-hidden transition-all duration-300 hover:shadow-xl"
                style={{ borderTopColor: platform.color, borderTopWidth: 3 }}
              >
                {/* Top accent bar */}
                <div className="absolute top-0 left-0 right-0 h-0.5 rounded-t-3xl" style={{ background: platform.color }} />
                {/* Glow on hover */}
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-2xl font-black mb-3" style={{ color: platform.color }}>{platform.name}</h3>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-6">{platform.desc}</p>

                  <div className="pt-5 border-t border-slate-100 dark:border-slate-800">
                    <p className="text-3xl font-black" style={{ color: platform.color }}>{platform.stat}</p>
                    <p className="text-xs text-slate-500 font-semibold mt-1 uppercase tracking-wider">{platform.statLabel}</p>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      ═══════════════════════════════════════════════ */}
      <section className="py-20 relative bg-white dark:bg-[#0B0C10]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative mt-12 mb-20">
            ...
            <div className="block md:hidden absolute left-6 top-0 bottom-0 w-[2px] bg-yellow-400 dark:bg-yellow-500" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-4 relative z-10">
              {PROCESS_STEPS.map((step, i) => {
                ...
                return (
                  ...
                  >
                    <h3 className={`w-32 md:w-auto text-lg md:text-xl font-bold uppercase tracking-widest md:mb-6 pt-1 md:pt-0 ${isActive ? "text-yellow-500" : "text-slate-900 dark:text-white"}`}>
                      PHASE {step.step}
                    </h3>
                    <div className={`shrink-0 z-10 flex items-center justify-center bg-yellow-400 dark:bg-yellow-500 rounded-full mx-6 md:mx-auto md:mb-6 group-hover:scale-110 transition-transform duration-300 ${isActive ? "w-10 h-10 md:w-12 md:h-12" : "w-6 h-6 md:w-8 md:h-8 my-1 md:my-2"}`} />
                    <div className="flex flex-col items-start md:items-center px-2">
                      <p className="text-slate-900 dark:text-white text-base font-extrabold uppercase tracking-wider mb-2">{step.title}</p>
                      <p className="text-slate-500 dark:text-slate-400 text-xs md:text-sm font-medium leading-relaxed max-w-[220px]">{step.desc}</p>
                    </div>
                  </motion.div>
                );
              })}
            ...
            <div className="grid lg:grid-cols-2">
                ...
                <div className="space-y-8">
                  {[
                    ...
                    <div key={i} className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800/50 pb-4">
                      <span className="text-slate-500 dark:text-slate-500 font-semibold uppercase tracking-wider text-xs sm:text-sm">{item.label}</span>
                      <span className="text-lg sm:text-xl font-bold text-slate-400 dark:text-slate-600">{item.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-10 lg:p-16 relative bg-gradient-to-br from-yellow-50 to-white dark:from-[#1A1A12] dark:to-[#11121A] overflow-hidden group">
                ...
                <h3 className="text-2xl md:text-3xl font-black mb-12 text-yellow-600 dark:text-yellow-400 flex items-center gap-3 relative z-10">
                  <Zap className="w-8 h-8 text-yellow-500" /> AlphaDigify Managed
                </h3>
                ...
                <div className="space-y-8 relative z-10">
                  {[
                    ...
                    >
                      <span className="text-slate-700 dark:text-slate-300 font-bold uppercase tracking-wider text-xs sm:text-sm">{item.label}</span>
                      <span className="text-xl sm:text-3xl font-black text-yellow-500 dark:text-yellow-400">{item.value}</span>
                    </motion.div>
                  ))}
                </div>
              </div>
      ...
      <section className="py-14 relative bg-white dark:bg-[#0B0C10] border-t border-slate-200 dark:border-slate-800/80">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <p className="text-slate-600 dark:text-slate-400 mb-6 text-lg">Ready to dominate every platform with bulletproof review authority?</p>
            <Link href="/contact">
              <Button size="lg" className="bg-yellow-500 hover:bg-yellow-400 text-slate-900 h-14 px-10 text-base font-bold rounded-full transition-all hover:scale-105 shadow-lg shadow-yellow-500/20">
